...
$('body').on('click', '.area-list p', function () {
  ...
  setCookies('goods_city_id', city_id, 30 * 24 * 60 * 60 * 1000);
...
function ajaxProvince(){
  $.ajax({
    type : "get",
    url: "/index.php?g=App&m=Api&a=getProvince",
    dataType:"json",
...
function ajaxCity(parent_id) {
  $.ajax({
    type: "get",
    url: "/index.php?g=App&m=Api&a=getRegionByParentId",
    dataType: "json",
...
function ajaxDispatching() {
  ...
  if(typeof(goods_id) != 'undefined' && region_id!= ''){
    $.ajax({
      ...
      url: "/index.php?g=App&m=Api&a=dispatching",
...
function locationInitialize(){
  ...
  city_id = getCookieByName('goods_city_id');
