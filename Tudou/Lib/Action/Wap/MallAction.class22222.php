...
class MallAction extends CommonAction{
    public function _initialize(){
        ...
        $this->assign('cartnum', (int) array_sum($goods_spec));
    ...
    public function buy($goods_id){
        ...
        session('goods_spec', $goods_spec, 604800);
   ...
   public function get_spec($goods_id){
     ...
     public function cartdel(){
        ...
        $goods_spec_all = session('goods_spec');
        if (isset($goods_spec_all[$goods_spec])) {
            unset($goods_spec_all[$goods_spec]);
            session('goods_spec', $goods_spec_all, 604800);
 	...
 	foreach($goods_ids as $v){
       ...
       if (empty($goods)) {
        ...
        foreach ($ordergoods as $k => $val) {
		 ...
		 $goods_spec_all = session('goods_spec');
         if(isset($goods_spec_all)) {
            unset($goods_spec_all);
            session('goods_spec', $goods_spec_all, 604800);
        } 
    ...
    public function pay2(){
        ...
        if ($code == 'wait') {
            //如果是货到付款
            $obj ->save(array('order_id' => $order_id, 'is_daofu' => 1, 'status' => 1));
            ...
            $need_pay = $obj ->useIntegral($this->uid, array($order_id));//更新支付结果,这里加了配送费
            if (empty($logs)) {
                $logs = array(
					'type' => 'goods', 
					'user_id' => $this->uid, 
