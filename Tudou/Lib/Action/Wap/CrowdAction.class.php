...
class CrowdAction extends CommonAction {
	...
	public function _initialize(){
        parent::_initialize();
		if(empty($this->_CONFIG['operation']['crowd'])){
			$this->error('众筹功能已关闭');die;
		}
  ...
  public function buy(){
		...
		if(empty($Crowdtype)){
			$this->tuMsg('获取众筹信息有误，请刷新页面后稍后再试试');
		}
       ...
       if ($order_id = D('Crowdorder')->add($data)) {
		   if(!empty($address_id)){
			   $this->tuMsg('恭喜您众筹成功，正在为您跳转', U('crowd/pay', array('order_id' => $order_id,'address_id'=>$address_id)));
		   }else{
			   $this->tuMsg('您暂无收货地址！,正在为您跳', U('address/addrcat', array('order_id' => $order_id,'type'=>crowd)));   
		   }
       }
       $this->tuMsg('创建订单失败');
	 ...
	 public function pay2(){
        ...
        if ($code == 'wait') {
			...
			if (false == D('Crowd')->add_crowd_list($order_id,$this->uid)) {//后期下单后更新，更新购买列表信息
				...
				$this->tuMsg('选择支付方式成功！,正在为您跳', U('payment/payment', array('log_id' => $logs['log_id'])));   
			}
