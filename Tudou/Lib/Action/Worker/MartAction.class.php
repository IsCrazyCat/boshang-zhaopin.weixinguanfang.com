            ...
            if (!($order = D('Order')->find($order_id))) {
                ...
                if ($order['shop_id'] != $this->shop_id) {
                    ...
                    if ($order['status'] == 2 || $order['status'] == 3 || $order['status'] == 8 || $order['status'] == 4 || $order['status'] == 5) {
						...
						D('Weixinmsg')->weixinTmplOrderMessage($order_id,$cate = 2,$type = 2,$status = 1);//通知买家
                        if ($this->_get('wait')) {
                            $this->tuMsg('恭喜您，货到付款发货成功', U('mart/wait2'));
                        } else {
                            $this->tuMsg('恭喜您，一键发货成功', U('mart/wait'));
                        }
                    }
                }
            }
    ...
    public function refund($order_id = 0){
        ...
        if ($detail['is_daofu'] == 0) {
			...
			if(false !== $order->implemented_refund($order_id)){
               $this->tuMsg('退款成功', U('mart/order',array('st'=>5)));
            }else{
                $this->tuMsg('退款失败');
            }
        }else{
	...
	public function changePrice($order_id = 0){
		...
		if($this->isPost()){
			...
			if(false != D('OrderChangeLogs')->orderChangePrice($order_id,$change_price)){
				$this->tuMsg('改价成功', U('mart/order',array('st'=>0)));
			}else{
