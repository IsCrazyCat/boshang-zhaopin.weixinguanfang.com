   ...
   //酒店申请退款
	public function refund(){
        ...
        if (!$hotel_order) {
            ...
            if ($hotel_order['user_id'] != $this->uid) {
                $this->tuMsg('请不要操作他人的订单');
            }elseif($hotel_order['order_status'] != 1){
    ...
    public function cancel_refund(){
        ...
        if (!$hotel_order) {
            ...
            if ($hotel_order['user_id'] != $this->uid) {
				...
				$Hotelorder->where('order_id =' . $order_id)->setField('order_status', 1);
				$this->tuMsg('酒店取消退款成功！',U('hotel/index', array('st' =>1)));
			}
        ...
        if(!$order_id = (int) $order_id){
            ...
            if ($this->_Post()) {
				...
				if (!$Hotel = D('Hotel')->find($detail['hotel_id'])) {
                    $this->tuMsg('没有找到对应的酒店，暂时无法点评，请稍后再试');
                }
                ...
                if ($comment_id = D('Hotelcomment')->add($data)) {
                    ...
                    $this->tuMsg('恭喜您点评成功!', U('hotels/index'));
                }
                $this->tuMsg('点评失败');
            }else {
                $this->assign('detail', $detail);
