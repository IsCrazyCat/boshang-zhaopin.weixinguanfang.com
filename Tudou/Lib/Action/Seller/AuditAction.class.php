...
class AuditAction extends CommonAction{
    ...
    public function index(){
        ...
        if ($this->isPost()) {
            ...
            if (!$photo) {
                $this->tuMsg('请上传营业执照，可以用手机拍照上传');
            }
            if (!isImage($photo)) {
                $this->tuMsg('所上传的营业执照格式不正确');
            }
            $pic = $this->_post('pic', false);
            if (!$pic) {
                $this->tuMsg('请上传员工身份证，可以用手机拍照上传');
            }
            if (!isImage($pic)) {
                $this->tuMsg('所上传员工身份证格式不正确');
            }
        ...
        if ($audit_id = (int) $audit_id) {
            $obj = D('Audit');
                ...
                if (false !== $obj->save($data)) {
                    $this->tuMsg('编辑操作成功', U('audit/index'));
                }
                $this->tuMsg('操作失败');
            } else {
