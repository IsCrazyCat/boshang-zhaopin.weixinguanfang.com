...
class AppointAction extends CommonAction {
 	...
 	public function main(){
        ...
        switch ($order) {
            ...
            case 'v':
                ...
                break;
            default:
                $orderby = array('views' => 'desc', 'worker_id' => 'desc');
                break;
        }
		$this->assign('order', $order);
		//搜索二开结束
    ...
    public function detail($appoint_id) {
		...
		$url = D('city')->check_city_domain($appoint_city_id['0']['city_id'],$this->_NOWHOST,$this->_TU_DOMAIN);
		if(!empty($url)){
			header("Location:".$url);
		}
    ...
    public function create($appoint_id) {
        ...
        if ($order_id = D('Appointorder')->add($data)) {
			$this->tuSuccess('恭喜您预约家政服务成功，正在为您跳转付款', U('appoint/pay', array('order_id' => $order_id)));
        }
        $this->tuError('服务器繁忙');
    }
