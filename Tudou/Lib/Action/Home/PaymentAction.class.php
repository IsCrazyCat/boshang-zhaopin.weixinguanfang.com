...
class PaymentAction extends CommonAction {
    ...
    public function respond(){
        ...
        if(!empty($detail)){
            if($detail['type'] == 'ele'){
                ...
                $this->edu_success('恭喜您课程支付成功啦！', $detail);
    ...
    public function payment($log_id) {
        ...
        }elseif ($logs['type'] == "zhe") {//五折卡
            $url = U('zhe/pay', array('order_id' => $logs['order_id']));
        }elseif ($logs['type'] == "edu") {//五折卡
            $url = U('edu/pay', array('order_id' => $logs['order_id']));
        }
        $this->assign('url', $url);
