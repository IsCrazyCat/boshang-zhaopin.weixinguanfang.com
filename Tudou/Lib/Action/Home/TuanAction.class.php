...
class TuanAction extends CommonAction{
    ...
    public function detail(){
		...
		$url = D('city')->check_city_domain($tuan_city_id['0']['city_id'],$this->_NOWHOST,$this->_TU_DOMAIN);
		if(!empty($url)){
			 header("Location:".$url);
		}
    ...
    public function order(){
        ...
        if ($num <= 0 || $num > 99) {
            $this->tuError('请输入正确的购买数量');
        }
        //增加限购开始
        if ($num > $detail['xiangou'] && $detail['xiangou'] > 0) {
            $this->tuError('亲，每人只能购买' . $detail['xiangou'] . '份哦');
        }
        if ($detail['xiadan'] == 1) {
        ...
        if (!empty($order)) {
            ...
            if (false !== D('Tuanorder')->save($data)) {
                $this->tuSuccess('修改订单成功', U('tuan/pay', array('order_id' => $order)));
            }
            $this->tuError('修改订单失败');
        } else {
            $data = array(
            ...
            if ($order_id = D('Tuanorder')->add($data)) {
                ...
                //更新减掉库存
                $this->tuSuccess('创建订单成功', U('tuan/pay', array('order_id' => $order_id)));
            }
            $this->tuError('创建订单失败');
        }
    }
    ...
    public function pay2()
    {
        ...
        if ($code == 'wait') {
            ...
            if (D('Tuanorder')->save(array('order_id' => $order_id, 'status' => '-1'))) {
                ...
                $this->tuSuccess('恭喜您下单成功', U('tuan/yes', array('code' => $codestr)));
            } else {
                $this->tuError('您已经设置过该抢购为到店付了');
            }
        } else {
            ...
            $this->tuSuccess('选择支付方式成功！下面请进行支付', U('payment/payment', array('log_id' => $logs['log_id'])));
        }
    }
