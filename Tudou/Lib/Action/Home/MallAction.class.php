...
class MallAction extends CommonAction{
    ...
    public function index(){
		...
		$map['attr_value']  = array('IN',$attr_values);
		//$TpGoodsAttr = M('TpGoodsAttr')->where($map)->group('attr_value')->select();
		$TpGoodsAttr = M('TpGoodsAttr')->where($map)->select();
   ...
   public function get_spec($goods_id){
 	 ...
 	 foreach($goods_ids as $v){
        ...
        $is_spec_stock = is_spec_stock($val[goods_id],$val[sky],$num[$val['goods_spec']]);
       	if(!$is_spec_stock){
       	 	...
       	 	$msg = '亲！规格为<' . $val['key_name']. '>的工作库存不够了,只剩' .  $spec_one_num . '件了！';
       	    $this->ajaxReturn(array('status' => 'error', 'msg' => $msg));
        }
		
	    if ($val['num'] < $num[$val['goods_spec']]) {
	      	$msg ='亲！工作<' . $val['title'] . '>库存不够了,只剩' . $val['num'] . '件了！';
        	$this->ajaxReturn(array('status' => 'error', 'msg' => $msg));
	    }
	   }
	 ...
	 foreach($goods_ids as $v){
		...
		$val = reset($val); //二维数组 取第一个  
         ...
         $is_spec_stock = is_spec_stock($val[goods_id],$val[sky],$num[$val['goods_spec']]);
       	 if(!$is_spec_stock){
       	 	$spec_one_num =  get_one_spec_stock($val[goods_id],$val[sky]);
        	    $this->tuError('亲！规格为<' . $val['key_name']. '>的工作库存不够了,只剩' . $spec_one_num . '件了');
        	}
		
	        if ($val['num'] < $num[$val['goods_spec']]) {
	            $this->tuError('亲！工作<' . $val['title'] . '>库存不够了,只剩' . $val['num'] . '件了');
	        }
	   }
        ...
        if (count($order_ids) > 1) {
            ...
            $this->tuJump(U('mall/paycode', array('log_id' => $logs['log_id'])));
        } else {
            $this->tuJump(U('mall/pay', array('order_id' => $order_id, 'address_id' => $defaultAddress['id'])));
            //加入快递参数
        }
    }
    ...
    public function paycode2(){
        ...
        if ($code == 'wait') {//如果是货到付款
            ...
            $this->tuJump(U('mall/combine', array('log_id' => $detail['log_id'])));
        }
    }
    ...
    public function pay2(){
        ...
        if ($code == 'wait') {
            //如果是货到付款
            D('Order')->save(array('order_id' => $order_id, 'is_daofu' => 1, 'status' => 1));
        ...
        foreach ($ordergoods_ids as $k => $v) {
         ...
         $is_spec_stock = is_spec_stock($v[goods_id],$v[key],$v['num']);
            ...
            if ($goods_num['num'] < $v['num']) {
                $this->tuSuccess('工作ID' . $v['goods_id'] . '库存不足无法付款', U('home/mall/index'));
            }
        }
        return false;
    }
