...
class CrowdAction extends CommonAction {
	...
	public function ask($goods_id){
        ...
        if(empty($data['ask_c'])){
            $this->tuError("提问问题不能为空");
        }
		$data['dateline'] =	time();
		if ($ask_id = $Crowdask->add($data)) {
			$this->tuMsg('提交成功',U('crowd/detail',array('goods_id'=>$goods_id)));
		}
	...
	public function favorites($goods_id){
		...
		if($Crowdfollow->where($map)->find()){
			...
			$this->tuMsg('关注成功',U('crowd/detail',array('goods_id'=>$goods_id)));
		}
	}
	 //众筹点赞
	public function zan($goods_id){
		...
		if($Crowdfollow->where($map)->find()){
			...
			$this->tuMsg('点赞成功',U('crowd/detail',array('goods_id'=>$goods_id)));
		}
	}
	...
	public function report($goods_id){
		...
		if($Crowdreport->where($map)->find()){
			...
			$this->tuMsg('举恭喜您举报成功',U('crowd/detail',array('goods_id'=>$goods_id)));
		}
	...
	public function prove($goods_id){
		...
		if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $this->tuMsg('恭喜您证明成功',U('crowd/detail',array('goods_id'=>$goods_id)));
            }
            $this->tuMsg('操作失败');
        } else {
  ...
  public function buy(){
       ...
       if ($order_id = D('Crowdorder')->add($data)) {
		   if(!empty($address_id)){
			   $this->tuSuccess('恭喜您众筹成功，正在为您跳转', U('crowd/pay', array('order_id' => $order_id,'address_id'=>$address_id)));
		   }else{
			   $this->tuSuccess('您暂无收货地址！,正在为您跳', U('members/malladdress/create', array('order_id' => $order_id,'type'=>crowd)));   
		   }
       }
       $this->tuError('创建订单失败');
	...
	//去付款
	 public function pay2(){
        ...
        if (!($code = $this->_post('code'))) {
            $this->tuError('请选择支付方式');
        }
        $uaddr = D('Paddress')->where(array('id' => $order['address_id']))->find();
        if ($code == 'wait') {
             $this->tuError('暂不支持货到付款，请重新选择支付方式');
        } else {
            $payment = D('Payment')->checkPayment($code);
            if (empty($payment)) {
                $this->tuError('该支付方式不存在，请稍后再试试');
            }
			...
			if (false == D('Crowd')->add_crowd_list($order_id,$this->uid)) {//后期下单后更新，更新购买列表信息
				$this->tuError('更新购买信息出错');
			}else{
				$this->tuJump(U('payment/payment', array('log_id' => $logs['log_id'])));
			}
