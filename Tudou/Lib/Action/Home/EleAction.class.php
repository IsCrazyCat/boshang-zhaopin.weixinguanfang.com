...
class EleAction extends CommonAction{
    ...
    public function delete2($product_id) {
        ...
        $this->tuSuccess('删除成功', U('ele/cart'));
    }
    ...
    public function delete($product_id){
        ...
        $this->_getcookie($product_id);
        $this->tuSuccess('删除成功', U('ele/shop', array('shop_id' => $detail['shop_id'])));
    }
    public function clean(){
        $shop_id = (int) $this->_param('shop_id');
        setcookie('eleproducts');
        $this->tuSuccess('清空购物车成功', U('ele/shop', array('shop_id' => $shop_id)));
    }
    ...
    public function add($product_id){
        ...
        if (!empty($this->cart)) {
            foreach ($this->cart as $k => $v) {
                ...
                if ($data['shop_id'] != $detail['shop_id']) {
                    $this->tuError('一次只能订购一家的外卖，您可以清空购物车重新定');
                }
                break;
            }
        }
        ...
        $this->tuSuccess('增加购物车成功', U('ele/shop', array('shop_id' => $detail['shop_id'])));
    }
    ...
    public function dianping()
    {
        ...
        if ($dianping_id = D('Shopdianping')->add($data)) {
            ...
            $this->tuSuccess('恭喜您点评成功!', U('ele/detail', array('shop_id' => $shop_id)));
        }
        $this->tuError('点评失败');
    }
}