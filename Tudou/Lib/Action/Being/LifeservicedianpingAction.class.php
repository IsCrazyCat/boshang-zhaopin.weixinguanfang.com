...
class LifeservicedianpingAction extends CommonAction {
    ...
    public function index() {
        ...
        import('ORG.Util.Page'); // 导入分页类 二开qq 120--585--022   www.juhucms.com
        $map = array('closed' => 0);
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                $this->tuSuccess('添加成功', U('Lifeservicedianping/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck() {
        ...
        $data['id'] = (int) $data['id'];
        if (empty($data['id'])) {
            $this->tuError('设计师ID不能为空');
        }
        if (!$order = D('Houseworksetting')->find($data['id'])) {
            $this->tuError('设计师ID不存在');
        }
        ...
        if ($id = (int) $id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->tuSuccess('操作成功', U('Lifeservicedianping/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    private function editCheck() {
	 ...
	 public function delete($id = 0) {
        if (is_numeric($id) && ($id = (int) $id)) {
            ...
            $this->tuSuccess('删除成功2', U('Lifeservicedianping/index'));
        } else {
            ...
            if (is_array($id)) {
                ...
                $this->tuSuccess('删除成功', U('Lifeservicedianping/index'));
            }
            $this->tuError('请选择要删除的设计师点评');
        }
    }
