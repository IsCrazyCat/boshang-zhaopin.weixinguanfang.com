...
class SmsshopAction extends CommonAction{
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            $obj = D('Smsshop');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('smsshop/index'));
            }
            $this->tuError('操作失败');
        } else {
    ...
    private function createCheck(){
		...
		if($data['user_id']){
			...
			if (D('Users')->getUserByAccount($data['account'])) {
				$this->tuError('该会员账户已经存在');
			}
			if($detail = D('Smsshop')->where(array('type' => user, 'user_id' => $data['user_id']))->find()){
				$this->tuError('该会员已经有短信记录');
			}	
			$data['type'] = user;
		}
        ...
        if ($log_id = (int) $log_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('smsshop/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($log_id = 0){
        if (is_numeric($log_id) && ($log_id = (int) $log_id)) {
            $obj = D('Smsshop');
            $obj->delete($log_id);
            $this->tuSuccess('删除成功', U('smsshop/index'));
        } else {
