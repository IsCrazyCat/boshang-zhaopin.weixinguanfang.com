...
class AllAction extends CommonAction {
    ...
    public function load(){
		...
		if(isWondows()){
			$keyword = htmlspecialchars(auto_charset($_REQUEST['keyword']));
		}else{
			$keyword = $this->_param('keyword', 'htmlspecialchars');
		}
        ...
        if ($keyword) {
			...
			$where_ele['shop_name'] = array('LIKE', '%' . $keyword . '%');
		...
		$list_ele=$Ele->Field('"外卖" as t_name,"ele/shop" as t_url,"shop_id" as t_param,shop_id as t_id,shop_name as t_title,photo as t_photo,sold_num as t_note')->order($orderby)->where($where_ele)->select();
		...
		$list_shop =$Shop->Field('"企业" as t_name,"shop/detail" as t_url,"shop_id" as t_param,shop_id as t_id,shop_name as t_title,photo as t_photo,tel as t_note')->order($orderby)->where($where_shop)->select();
