...
class TuandianpingAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                $this->tuSuccess('添加成功', U('tuandianping/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck() {
        ...
        if (empty($data['order_id'])) {
            $this->tuError('抢购订单不能为空');
        }
        if (!$order = D('Tuanorder')->find($data['order_id'])) {
            $this->tuError('抢购订单不存在');
        }
       ...
       public function edit($order_id = 0) {
		...
		if (false !== D('Shop')->fenzhan_check_city_id($detail['shop_id'],$this->city_id)) {
           $this->tuError('非法操作', U('tuandianping/index'));
        }
		
            $obj = D('Tuandianping');
            if (!$detail = $obj->find($order_id)) {
                $this->tuError('请选择要编辑的抢购点评');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->tuSuccess('操作成功', U('tuandianping/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    private function editCheck() {
	 ...
	 public function delete($order_id = 0) {
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            $this->tuSuccess('删除成功2', U('tuandianping/index'));
        } else {
            ...
            if (is_array($order_id)) {
                ...
                $this->tuSuccess('删除成功', U('tuandianping/index'));
            }
