...
class MalldianpingAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                $this->tuSuccess('添加成功', U('malldianping/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck() {
        ...
        if (empty($data['order_id'])) {
            $this->tuError('商城订单不能为空');
        }
        if (!$order = D('Eleorder')->find($data['order_id'])) {
            $this->tuError('商城订单不存在');
        }
		...
		//查询上级ID编辑处代结束
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->tuSuccess('操作成功', U('malldianping/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    private function editCheck() {
	 ...
	 public function delete($order_id = 0) {
		...
		//查询上级ID编辑处代结束
            ...
            $this->tuSuccess('删除成功1', U('malldianping/index'));
        } else {
            ...
            if (is_array($order_id)) {
                ...
                $this->tuSuccess('批量删除成功', U('malldianping/index'));
            }
            $this->tuError('请选择要删除的商城点评');
        }
    }
