...
class MarketAction extends CommonAction
{
    ...
    private function createCheck()
    {
        ...
        if (empty($data['market_name'])) {
            $this->tuError('商场名称不能为空');
        }
        $data['logo'] = htmlspecialchars($data['logo']);
        if (empty($data['logo'])) {
            $this->tuError('请上传商场LOGO');
        }
        if (!isImage($data['logo'])) {
            $this->tuError('商场LOGO格式不正确');
        }
        $data['photo'] = htmlspecialchars($data['photo']);
        if (empty($data['photo'])) {
            $this->tuError('请上传商场缩略图');
        }
        if (!isImage($data['photo'])) {
            $this->tuError('商场缩略图格式不正确');
        }
        $data['addr'] = htmlspecialchars($data['addr']);
        if (empty($data['addr'])) {
            $this->tuError('商场地址不能为空');
        }
        $data['summary'] = htmlspecialchars($data['summary']);
        if (empty($data['summary'])) {
            $this->tuError('小编总结不能为空');
        }
        $data['tel'] = htmlspecialchars($data['tel']);
        if (empty($data['tel'])) {
            $this->tuError('商场电话不能为空');
        }
        if (!isPhone($data['tel']) && !isMobile($data['tel'])) {
            $this->tuError('商场电话格式不正确');
        }
        ...
        if ($market_id = (int) $market_id) {
            ...
            if ($this->isPost()) {
                ...
                if ($res = D('Marketfloor')->where(array('market_id' => $market_id, 'floor_name' => $data['floor_name']))->find()) {
                    $this->tuError('楼层名称已存在');
                }
                $data['market_id'] = $market_id;
                $data['orderby'] = (int) $this->_post('orderby');
                if (D('Marketfloor')->add($data)) {
                    $this->tuSuccess('添加楼层成功', U('market/floor', array('market_id' => $market_id)));
                }
            } else {
                $this->assign('detail', $detail);
                $this->display();
    ...
    public function flooredit($market_id, $floor_id)
    {
        ...
        if (!($detail = D('Market')->find($market_id))) {
            ...
            $this->tuError('您选择的商场不存在2');
        }
        if ($detail['closed'] == 1) {
            $this->tuError('您选择的商场不存在3');
        }
        ...
        if ($this->isPost()) {
            ...
            if (false !== D('Marketfloor')->save($data)) {
                $this->tuSuccess('编辑楼层成功', U('market/floor', array('market_id' => $market_id)));
            }
        } else {
            $this->assign('floor', $floor);
            $this->assign('detail', $detail);
    ...
    public function recovery($floor_id, $market_id)
    {
        if (is_numeric($floor_id) && ($floor_id = (int) $floor_id)) {
            ...
            $this->tuSuccess('恢复成功', U('market/floor', array('market_id' => $market_id)));
        }
    }
        ...
        if ($market_id = (int) $market_id) {
            ...
            if ($this->isPost()) {
                ...
                if ($enter_id = D('Marketenter')->add($data)) {
                    $this->tuSuccess('入驻成功', U('market/enterlist', array('market_id' => $market_id)));
                }
            } else {
