...
class ShopgradeAction extends CommonAction{
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->tuSuccess('添加成功', U('Shopgrade/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck(){
        ...
        if (empty($data['photo'])) {
            $this->tuError('等级图标不能为空');
        }
        $data['money'] = ((int) $data['money'])*100;
		if (empty($data['money'])) {
            $this->tuError('单独购买需要多少金额不能为空');
        }
        $data['gold'] = ((int) $data['gold'])*100;
		if (empty($data['gold'])) {
             $this->tuError('自动升级需要的商户资金不能为空');
        }
        ...
        if ($grade_id = (int) $grade_id){
            ...
            if($this->isPost()){
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->tuSuccess('操作成功', U('Shopgrade/index'));
                }
                $this->tuError('操作失败');
            }else{
    ...
    private function editCheck(){
		...
		$count = D('Shopgrade')->where(array('orderby'=>$data['orderby']))->count();
		if($count > 1){
            $this->tuError('等级权重重复，请调整后在提交');
        }
        return $data;
