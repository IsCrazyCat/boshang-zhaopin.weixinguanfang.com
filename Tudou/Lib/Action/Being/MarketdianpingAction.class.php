<?php
class MarketdianpingAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                if (!empty($local))
                    D('Marketdianpingpics')->upload($data['order_id'], $local);
                $this->tuSuccess('添加成功', U('marketdianping/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck() {
        ...
        if (empty($data['order_id'])) {
            $this->tuError('菜市场订单不能为空');
        }
        if (!$order = D('Marketorder')->find($data['order_id'])) {
            $this->tuError('菜市场订单不存在');
        }
        ...
        if ($order_id = (int) $order_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->tuSuccess('操作成功', U('marketdianping/index'));
                }
                $this->tuError('操作失败');
            } else {
                ...
                $this->assign('photos', D('Marketdianpingpics')->getPics($order_id));
                $this->display();
            }
        } else {
            $this->tuError('请选择要编辑的菜市场点评');
    ...
    private function editCheck() {
	 ...
	 public function delete($order_id = 0) {
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            $this->tuSuccess('删除成功', U('marketdianping/index'));
        } else {
