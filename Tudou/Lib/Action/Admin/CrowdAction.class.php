   ...
   public function order() {
    ...
    public function audit($goods_id = 0) {
        if (is_numeric($goods_id) && ($goods_id = (int) $goods_id)) {
            ...
            $this->tuSuccess('审核成功', U('crowd/index'));
        } else {
            $goods_id = $this->_post('goods_id', false);
            if (is_array($goods_id)) {
                $obj = D('Crowd');
	 ...
	 public function create() {
        if ($this->isPost()) {
            ...
            if ($goods_id = $obj->add($data)) {
                ...
                $this->tuSuccess('添加成功', U('crowd/index'));
            }
            $this->tuError('操作失败');
        } else {
        ...
        if ($goods_id = (int) $goods_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->tuSuccess('操作成功', U('crowd/index'));
                }
                $this->tuError('操作失败');
            } else {
		...
		if ($goods_id = (int) $goods_id) {
			...
			if ($data = $this->_post('data', false)) {
				if (!empty($data['price']) && !empty($data['content']) && !empty($data['max_num']) && !empty($data['fahuo'])){
					...
					$this->tuSuccess('操作成功', U('crowd/setting',array('goods_id'=>$detail['goods_id'])));
				}else{
					 $this->tuError('内容不能为空');
				}
			}else{
	...
	public function type_edit($type_id){
		if ($type_id = (int) $type_id) {
            ...
            if (!$detail = $Crowd->find($goods_id)) {
                $this->tuError('请选择要设置的众筹');
            }
            if ($detail['closed'] != 0) {
                $this->tuError('该众筹已被删除');
            }
            $Crowd_list = $Crowd->find($goods_id);			
			if ($data = $this->_post('data', false)) {
				if (!empty($data['price']) && !empty($data['content']) && !empty($data['max_num']) && !empty($data['fahuo'])){
		...
		if ($goods_id = (int) $goods_id) {
			...
			if ($data = $this->_post('data', false)) {
				if (!empty($data['content'])){
					...
					$this->tuSuccess('操作成功', U('crowd/project',array('goods_id'=>$detail['goods_id'])));
				}else{
					 $this->tuError('内容不能为空');
				}
			}else{
		...
		if ($ask_id = (int) $ask_id) {
			...
			if ($data = $this->_post('data', false)) {
				if (!empty($data['answer_c'])){
					...
					$this->tuSuccess('操作成功', U('crowd/ask',array('goods_id'=>$ask['goods_id'])));
				}else{
					 $this->error('内容不能为空');
				}
			}else{
	...
	//后台话题批量删除
	 public function ask_delete($ask_id = 0) {
        if (is_numeric($ask_id) && ($ask_id = (int) $ask_id)) {
            ...
            $obj->delete($ask_id);
			$obj->cleanCache();
            $this->tuSuccess('操作成功', U('crowd/ask',array('goods_id'=>$detail['goods_id'])));
        } else {
