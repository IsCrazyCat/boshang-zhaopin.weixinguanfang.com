<?php
class StoredianpingAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                if (!empty($local))
                    D('Storedianpingpics')->upload($data['order_id'], $local);
                $this->tuSuccess('添加成功', U('storedianping/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck() {
        ...
        if (empty($data['order_id'])) {
            $this->tuError('便利店订单不能为空');
        }
        if (!$order = D('Storeorder')->find($data['order_id'])) {
            $this->tuError('便利店订单不存在');
        }
        ...
        if ($order_id = (int) $order_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->tuSuccess('操作成功', U('storedianping/index'));
                }
                $this->tuError('操作失败');
            } else {
                ...
                $this->assign('photos', D('Storedianpingpics')->getPics($order_id));
                $this->display();
            }
        } else {
            $this->tuError('请选择要编辑的便利店点评');
    ...
    private function editCheck() {
	 ...
	 public function delete($order_id = 0) {
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            $this->tuSuccess('删除成功', U('storedianping/index'));
        } else {
