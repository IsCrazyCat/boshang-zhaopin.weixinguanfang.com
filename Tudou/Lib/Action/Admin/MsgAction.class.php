...
class MsgAction extends CommonAction
{
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            $obj = D('Msg');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('msg/index'));
            }
            $this->tuError('操作失败');
        } else {
    ...
    public function edit($msg_id = 0)
    {
        if ($msg_id = (int) $msg_id) {
            ...
            if (!($detail = $obj->find($msg_id))) {
                $this->tuError('请选择要编辑的手机消息');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                $data['msg_id'] = $msg_id;
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('msg/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($msg_id = 0){
        if (is_numeric($msg_id) && ($msg_id = (int) $msg_id)) {
            ...
            $this->tuSuccess('删除成功', U('msg/index'));
        } else {
            ...
            if (is_array($msg_id)) {
                ...
                $this->tuSuccess('批量删除成功', U('msg/index'));
            }
            $this->tuError('请选择要批量删除的手机消息');
        }
    }
