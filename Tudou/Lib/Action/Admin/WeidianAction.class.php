...
class WeidianAction extends CommonAction
{
    ...
    public function audit($wd_id = 0)
    {
        if (is_numeric($wd_id) && ($wd_id = (int) $wd_id)) {
            ...
            $this->tuSuccess('审核成功', U('weidian/index'));
        } else {
            $error = 0;
            ...
            if (is_array($wd_id)) {
                ...
                if ($error > 0) {
                    $this->tuSuccess($error . '条审核失败', U('weidian/index'));
                } else {
                    $this->tuSuccess('审核成功', U('weidian/index'));
                }
            }
            $this->tuError('请选择要审核的微店');
        }
    }
        ...
        if ($shop_id = (int) $shop_id) {
            ...
            if ($this->isPost()) {
                ...
                if ($robj) {
                    ...
                    $this->tuSuccess('操作成功', U('weidian/index'));
                } else {
                    $this->tuError('操作失败' . $obj->getLastSql());
                }
            } else {
                $this->assign('citys', D('City')->fetchAll());
                $this->assign('areas', D('Area')->fetchAll());
    ...
    private function editCheck($shop_id)
    {
        ...
        if (empty($data['pic'])) {
            $this->tuError('请上传微店形象照');
        }
        if (!isImage($data['pic'])) {
            $this->tuError('微店形象照格式不正确');
        }
        $data['logo'] = htmlspecialchars($data['logo']);
        if (empty($data['logo'])) {
            $this->tuError('请上传微店LOGO');
        }
        if (!isImage($data['logo'])) {
            $this->tuError('微店LOGO格式不正确');
        }
    ...
    public function delete($wd_id = 0)
    {
        if (is_numeric($wd_id) && ($wd_id = (int) $wd_id)) {
            ...
            $this->tuSuccess('删除微店成功', U('weidian/index'));
        } else {
            $wd_id = $this->_post('id', false);
            if (is_array($wd_id)) {
                $obj = D('WeidianDetails');
                ...
                $this->tuSuccess('批量删除微店成功', U('weidian/index'));
            }
            $this->tuError('请选择要批量删除的微店');
        }
    }
}