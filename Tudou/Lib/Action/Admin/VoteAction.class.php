...
class VoteAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            if ($vote_id = $obj->add($data)) {
                $this->tuSuccess('添加成功', U('vote/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function createCheck()
    {
        ...
        if (empty($data['title'])) {
            $this->tuError('投票标题不能为空');
        }
        $data['banner'] = htmlspecialchars($data['banner']);
        if (empty($data['banner'])) {
            $this->tuError('投票banner不能为空');
        }
        if (!isImage($data['banner'])) {
            $this->tuError('投票banner格式不正确');
        }
        ...
        if ($vote_id = (int) $vote_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('vote/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($vote_id = 0)
    {
        if (is_numeric($vote_id) && ($vote_id = (int) $vote_id)) {
            $obj = D('Vote');
            $obj->delete($vote_id);
            $this->tuSuccess('删除成功', U('vote/index'));
        } else {
    ...
    public function work($vote_id = 0)
    {
        if (is_numeric($vote_id) && ($vote_id = (int) $vote_id)) {
            ...
            if (empty($detail)) {
                $this->tuError('您还未设置投票选项');
            }
            $obj->save(array('vote_id' => $vote_id, 'is_work' => 1));
            $this->tuSuccess('启用成功', U('vote/index'));
        } else {
            $vote_id = $this->_post('vote_id', false);
            if (is_array($vote_id)) {
                $obj = D('Vote');
