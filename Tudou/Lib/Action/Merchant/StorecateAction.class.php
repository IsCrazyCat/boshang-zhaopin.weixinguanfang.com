<?php
class StorecateAction extends CommonAction
{
    ...
    public function create($parent_id = 0){
        if ($this->isPost()){
            ...
            if($obj->add($data)){
                $this->tuSuccess('添加成功', U('storecate/index'));
            }
            $this->tuError('操作失败');
        }else{
        ...
        if ($cate_id = (int) $cate_id) {
            ...
            if($this->isPost()){
                ...
                if(false !== $obj->save($data)){
                    $this->tuSuccess('操作成功', U('storecate/index'));
                }
                $this->tuError('操作失败');
            }else{
    ...
    public function dmarkette($cate_id = 0){
        if (is_numeric($cate_id) && ($cate_id = (int) $cate_id)) {
            ...
            if(!($detail = $obj->where(array('shop_id' => $this->shop_id, 'cate_id' => $cate_id))->find())) {
                $this->tuError('请选择要删除的便利店分类');
            }
			
			$res = D('Storeproduct')->where(array('cate_id'=>$cate_id))->find();
			if($res){
            ...
            $this->tuSuccess('删除成功', U('storecate/index'));
        }
