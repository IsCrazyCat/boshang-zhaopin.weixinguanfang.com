...
class FarmAction extends CommonAction {
    ...
    private function check_farm(){
        ...
        if(!$res){
            ...
            $this->error('您的农家乐已被删除请重新提交资料', U('farm/set_farm'));
        }elseif($res['audit'] == 0){
            ...
            if (empty($Farm)) {
                ...
                if($farm_id = $obj->add($data)){
                    ...
                    foreach($cate_id as $k=>$v){
                     ...
                     $this->tuSuccess('设置成功', U('farm/index'));
                }else{
                    $this->tuError('设置失败');
                }
            }else{
                ...
                if(false !== $obj->save($data)){
                   D('FarmPics')->where(array('farm_id'=>$farm_id))->delete();
                    ...
                    $this->tuSuccess('修改成功', U('farm/index'));
                }else{
                    $this->tuError('修改失败');
                }
            }
    ...
    public function setpackage(){ 
        ...
        if ($this->isPost()) {
            ...
            if ($farm_id = $fp->add($data)) {
                $this->tuSuccess('添加成功', U('farm/package'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    public function editpackage($pid=null){
        ...
        if ($pid = (int) $pid) {
            ...
            if ($this->isPost()) {
                ...
                $data['pid'] = $pid;
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('保存成功', U('farm/package'));
                }
                $this->tuError('操作失败');
            } else {
                $this->assign('detail',$detail);
    ...
    public function deletepackage($pid=null){
        ...
        if ($pid = (int) $pid) {
            ...
            if (false !== $obj->delete($pid)) {
                $this->tuSuccess('删除成功', U('farm/package'));
            }else {
                $this->tuError('删除失败');
            }
        } else {
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('FarmOrder')->find($order_id)){
                ...
                if(false !== D('FarmOrder')->cancel($order_id)){
                    $this->tuSuccess('订单取消成功',U('farm/index'));
                }else{
                    $this->tuError('订单取消失败');
                }
            }
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('FarmOrder')->find($order_id)){
                ...
                if(false !== D('FarmOrder')->complete($order_id)){
                    $this->tuSuccess('订单操作成功',U('farm/index'));
                }else{
                    $this->tuError('订单操作失败');
                }
            }
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('FarmOrder')->find($order_id)){
                ...
                if(false !== D('FarmOrder')->save(array('order_id'=>$order_id,'closed'=>1))){
                    ...
                    $this->tuSuccess('订单删除成功',U('farm/index'));
                }else{
                    $this->tuError('订单删除失败');
                }
            }
