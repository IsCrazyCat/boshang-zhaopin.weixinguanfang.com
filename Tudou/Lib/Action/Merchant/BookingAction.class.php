...
class BookingAction extends CommonAction{
    ...
    public function roomcreate(){
         ...
         if(IS_POST){
             ...
             if($room_id = $obj->add($data)){
				 $qrcode = D('Bookingroom')->getQrCode($room_id); 
				 D('Bookingroom')->where(array('room_id'=>$room_id))->save(array('qrcode'=>$qrcode));
                 $this->tuSuccess('恭喜你操作成功',U('booking/room'));
             }
             $this->tuError('操作失败');
         }else{             
             $this->assign('types',$obj->getType());
    ...
    public function roomdelete($room_id){
        ...
        if($room_id = (int)$room_id){
            ...
            $data['room_id'] = $room_id;
            if(false != $obj->save($data)){
                $this->tuSuccess('操作成功',U('booking/room'));
            }
            $this->tuError('操作失败');
        }else{
            $this->tuError('参数错误');
            ...
            if (empty($booking)) {
                ...
                if($obj->add($data)){
                    ...
                    foreach($type as $k=>$val){
                     ...
                     $this->tuSuccess('设置成功', U('booking/index'));
                }else{
                    $this->tuError('设置失败');
                }
            }else{
                ...
                if(false !== $obj->save($data)){
                    ...
                    $this->tuSuccess('修改成功', U('booking/index'));
                }else{
                    $this->tuError('修改失败');
                }
            }
    ...
    public function catecreate() {
        if ($this->isPost()) {
            ...
            $obj = D('Bookingcate');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('booking/cate'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    public function cateedit($cate_id = 0){
        if ($cate_id = (int) $cate_id){
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('booking/cate'));
                }
                $this->tuError('操作失败');
            } else {
                $this->assign('detail', $detail);
    ...
    public function catedelete($cate_id = 0) {
        if (is_numeric($cate_id) && ($cate_id = (int) $cate_id)) {
            ...
            if (!$detail = $obj->where(array('shop_id' => $this->shop_id, 'cate_id' => $cate_id))->find()) {
                $this->tuError('请选择要删除的菜品分类');
            }
            $obj->delete($cate_id);
            $this->tuSuccess('删除成功',U('booking/cate'));
        }
        $this->tuError('请选择要删除的菜品分类');
    }
    ...
    public function menucreate() {
        if ($this->isPost()) {
            ...
            $obj = D('Bookingmenu');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('booking/menu'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    private function menuCreateCheck() {
        ...
        if (empty($data['menu_name'])) {
            $this->tuError('菜品名称不能为空');
        }
        $data['shop_id'] = $this->shop_id;
        $data['cate_id'] = (int) $data['cate_id'];
        if (empty($data['cate_id'])) {
            $this->tuError('菜品分类不能为空');
        }
        $data['photo'] = htmlspecialchars($data['photo']);
        if (empty($data['photo'])) {
            $this->tuError('请上传缩略图');
    ...
    public function menuedit($menu_id = 0){
        if($menu_id = (int) $menu_id){
            ...
            if ($this->isPost()){
                ...
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('booking/menu'));
                }
                $this->tuError('操作失败');
            }else{
                $this->assign('detail', $detail);
    ...
    public function menudelete($menu_id = 0){
        if (is_numeric($menu_id) && ($menu_id = (int) $menu_id)){
            ...
            if (!$detail = $obj->where(array('shop_id' => $this->shop_id, 'menu_id' => $menu_id))->find()) {
                $this->tuError('请选择要删除的菜品设置');
            }
            $obj->save(array('menu_id' => $menu_id, 'closed' => 1));
            $this->tuSuccess('删除成功', U('booking/menu'));
        }
        $this->tuError('请选择要删除的菜品设置');
    }
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('Bookingorder')->find($order_id)){
                ...
                if(false !== D('Bookingorder')->cancel($order_id)){
                    $this->tuSuccess('订单取消成功',U('booking/index'));
                }else{
                    $this->tuError('订单取消失败');
                }
            }
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('Bookingorder')->find($order_id)){
                ...
                if(false !== D('Bookingorder')->complete($order_id)){
                    $this->tuSuccess('订单操作成功',U('booking/index'));
                }else{
                    $this->tuError('订单操作失败');
                }
            }
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('Bookingorder')->find($order_id)){
                ...
                if(false !== D('Bookingorder')->delete($order_id)){
                    ...
                    $this->tuSuccess('订单删除成功',U('booking/index'));
                }else{
                    $this->tuError('订单删除失败');
                }
            }
    ...
    public function setting(){
        ...
        if(IS_POST){
            ...
            if(!isMobile($data['mobile'])){
                $this->tuError('请填写正确的手机号码');
            }
            $data['money'] = (int)($_POST['data']['money']* 100);
			if(empty($data['money'])){
				$this->tuError('定金不能为空或者为0');
			}
            ...
            $obj->save($data);
            $this->tuSuccess('设置成功',U('booking/setting'));
        } else{
