   ...
   public function create() {
			...
			if ($id = $Paddress->add($newadd)) {
				if($data['type'] == goods && (!empty($id))){//商城类型
					...
					if(!empty($create_order_id)){//首先检测订单ID
						...
						$this->tuSuccess('添加地址操作成功', U('malladdress/index'));
					}
				}elseif($data['type'] == crowd && (!empty($id))){//众筹类型
					$this->tuSuccess('添加众筹地址成功，正在为您返回', U('home/crowd/detail', array('goods_id' =>$create_goods_id)));
				}else{
					$this->tuSuccess('添加地址操作成功，但是没识别到类型', U('malladdress/index'));
				}
			} else {
				$this->tuError('操作失败');
			}
	...
	public function edit() {
	    ...
	    if ($this->isPost()) {
			...
			if ($Paddress->save($newadd)) {
				if(empty($order_id)){
					$this->tuSuccess('修改收获地址成功', U('malladdress/index'));
				}else{
					D('Ordergoods')->update_express_price($this->uid,$data['type'], $order_id,$id);//去更新订单运费
					$this->tuSuccess('修改成功', U('home/mall/pay', array('type' => $data['type'],'order_id' => $order_id,'address_id'=>$id)));
				}
				
			} else {
				$this->tuError('修改失败，请重试');
			}
    ...
    public function delete($id){
        ...
        if ($detail['user_id'] != $this->uid) {
            $this->tuError('请不要操作别人的收货地址');
        }
        $obj = D('Paddress');
        $obj->save(array('id' => $id, 'closed' => 1));
        $this->tuSuccess('删除成功', U('malladdress/index'));
    }
