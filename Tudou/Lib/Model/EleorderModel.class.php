...
class EleorderModel extends CommonModel {
		...
		if($detail = D('Eleorder')->find($order_id)){
			if($detail['status'] != 2){
				...
				if (D('Eleorder')->save(array('order_id' => $order_id, 'status' => 8,'end_time' => NOW_TIME))) { //防止并发请求
					...
					D('Shopmoney')->insertData($order_id,$id ='0',$detail['shop_id'],$detail['settlement_price'],$type ='ele',$Intro);//结算给企业
					D('Eleorder')->AddDeliveryIogistics($order_id);//结算配送费给配送员
	...
	public function AddDeliveryIogistics($order_id){
		...
		if($detail = D('Eleorder')->find($order_id)){
			...
			//首先是配送状态
			if($shop['is_ele_pei'] == 1){
				if($do['logistics_price'] > 10){
					$logistics = $do['logistics_price'];
				}else{
					$logistics = $shop['express_price'];
				}
				D('Runningmoney')->add_delivery_logistics($order_id,$logistics,1);//配送费接口
				return true;
			}else{
				return true;
			}
