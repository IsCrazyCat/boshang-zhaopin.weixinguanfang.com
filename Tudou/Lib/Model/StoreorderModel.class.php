		...
		if($detail = D('Storeorder')->find($order_id)){
			if($detail['status'] != 2){
				...
				if (D('Storeorder')->save(array('order_id' => $order_id, 'status' => 8,'end_time' => NOW_TIME))){ //防止并发请求
					$Intro = '便利店订单结算';
					D('Shopmoney')->insertData($order_id,$id ='0',$detail['shop_id'],$detail['settlement_price'],$type ='store',$Intro);//结算给企业
					...
					D('Storeorder')->AddDeliveryIogistics($order_id);//结算配送费给配送员
		...
		if($detail = D('Storeorder')->find($order_id)){
			...
			if($Shop['is_store_pei'] == 1){
				if($detail['logistics_full_money']){//如果扣除的配送费一样分成
					...
					D('Runningmoney')->add_delivery_logistics($order_id,$store['logistics'],1);//配送费接口
					return true;
				}
			}else{
				return true;
