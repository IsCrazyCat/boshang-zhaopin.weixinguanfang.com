...
class CrowdModel extends CommonModel{
	...
	public function add_crowd_list($order_id, $user_id){
	 ...
	 //这里是去更已众筹的各种状态逻辑
	public function update_crowd_order_status($order_id){
		$order = D('Crowdorder')->find($order_id);//订单信息
		$type = D('Crowdtype')->where(array('type_id'=>$order['type_id']))->find();
	    D('Crowd')->updateCount($order['goods_id'], 'have_price',$order['price']);
		D('Crowd')->updateCount($order['goods_id'], 'have_num');
		D('Crowdlist')->updateCount($type['type_id'], 'have_num');
        return TRUE;
