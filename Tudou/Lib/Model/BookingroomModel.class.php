...
class BookingroomModel extends CommonModel{
	...
	//生成房间二维码
	public function getQrCode($room_id){
		$res = D('Bookingroom')->where(array('room_id'=>$room_id))->find();
		$url = U('wap/booking/menu', array('room_id'=>$room_id,'shop_id'=>$res['shop_id'],'t' => NOW_TIME, 'sign' => md5($room_id . C('AUTH_KEY') . NOW_TIME)));
        $token = 'booking_room_id_' . $room_id;
