    ...
    public function zan($shop_id, $user_id){
		...
		if($config['shop']['day_shop_zan_num']){
			...
			if($day_shop_zan_num >= $config['shop']['day_shop_zan_num']){
				$this->error = '您对企业【'.$Shop['shop_name'].'】点赞太频繁了，系统限制每天对同一个企业最多点赞【'.$config['shop']['day_shop_zan_num'].'】次';
				return false;
			}
		}
        ...
        if($this->add($data)){
			...
			if($config['shop']['zan_reward_integral']){
				D('Users')->addIntegral($user_id, $config['shop']['zan_reward_integral'], '给企业【'.$Shop['shop_name'].'】点赞奖励积分');
			}
			return true;
        }else{
			$this->error = '操作失败';
