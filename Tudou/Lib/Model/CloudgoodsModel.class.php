...
class CloudgoodsModel extends CommonModel{
    ...
    public function pay_cloud($goods_id, $user_id, $num,$log_id){
	 ...
	 //云购在线付款回调
    public function save_cloud_order_status($log_id){
		$obj = D('Cloudlogs');
    ...
    public function lottery($goods_id){
        ...
        if(false !== $this->save(array(
            ...
            if(!empty($detail['shop_id'])){
                ...
                D('Users')->addGold($shops['user_id'], $detail['settlement_price'],$intro);
            }
			D('Sms')->sms_cloud_win_user($goods_id,$list[$zhongjiang]['user_id'],$list[$zhongjiang]['number']);//云购中奖短信通知会员
            return true;
        }
    }
}