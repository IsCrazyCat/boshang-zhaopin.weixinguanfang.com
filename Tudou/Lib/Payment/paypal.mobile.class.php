...
class paypal{
	...
	*/ 
    ...
    function getCode($logs,$payment){
		...
		// 如果为空或不传递该参数，则不会跳转 
		$arr['return']	= $config['site']['host'] . U('wap/payment/respond', array('code'=>'paypal','id'=>$logs['logs_id']));	
		...
		$arr['cancel_return']	= $config['site']['host'] . U('wap/payment/respond', array('code'=>'paypal','id'=>$logs['logs_id']));	
		// Paypal会将指定 invoice 的订单的状态定时发送到此URL 
		// (Paypal的此操作，是paypal的服务器和我方商城的服务器点对点的通信，用户感觉不到）
		$arr['notify_url']	= $config['site']['host'] . U('wap/payment/respond', array('code'=>'paypal','id'=>$logs['logs_id'])); 
		$arr['rm']	= '2'; 
		
		$paypal_payment_url = $this->gateway.http_build_query($arr); 
		$button = '<input type="button" class="button button-block bg-green button-big" onclick="window.open(\''.$paypal_payment_url.'\')" value=" 立刻使用paypal支付 " />';
        return $button;
    }
